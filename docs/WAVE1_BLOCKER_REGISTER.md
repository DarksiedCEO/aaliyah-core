...
## W1.3 R2 — LEDGER INTEGRITY (Track B, work order docs/W13_R2_R4_WORK_ORDER.md)

Builder session, sole writer. Every EXECUTED-BY-GATE premise was re-executed
at the SHA before its item was touched; results are in
`aaliyah-w13-evidence/r2/probes/`.

### PREMISES RE-EXECUTED

| premise | at | result |
|---|---|---|
| D-03 L5-control (wrong digest) | 5fe8ea0 | REFUSED — correct, unchanged |
| D-03 L4 (phantom rows 057..060, no digest) | 5fe8ea0 | ACCEPTED — reproduced, fail OPEN |
| D-03 L4b (phantom rows with TRUE digests) | 5fe8ea0 | ACCEPTED — reproduced, fail OPEN |
| D-03 L5 / I-5 P3b (digest NULLed) | 5fe8ea0 | ACCEPTED and re-blessed — reproduced |
| I-5 pre-057 upgrade | 5fe8ea0 | ACCEPTED, 60/60 digested, no attestation — reproduced |
| I-6 B5 (emptied ledger) | 5fe8ea0 | refused by a DDL collision, not a guard — reproduced |
| RT4-3 MUT-B (lock after ledger) | 37163d6 | SURVIVED, replay 18/18 — reproduced |
| R-04 (bare pool, lock held 125s) | 1202fc7 | REJECTED 55P03 at 120,021ms — reproduced |
| R-04 on the PRODUCTION pool | 1202fc7 | **REJECTED 57014 at 30,018ms — worse than the gate recorded** |

### PRODUCTION CHANGE 1 — `faa2ae1`: the ledger is read back against the schema

Files: `src/persistence/postgres/migrations.ts`, `migrationCatalog.ts` (new),
`migrationEvidence.generated.ts` (new, generated by
`scripts/migration-evidence.ts`), migration **061** (new, ledger attestation
columns + whole-or-absent CHECK), `scripts/attest-migration-ledger.ts` (new).

- Each migration's catalog effect is recorded BY EXECUTION (every migration's
  SQL applied in order on an empty database, catalog diffed after each). The
  runner reads the schema back against it before applying and after.
- A build whose evidence was generated from different SQL is refused.
- An undigested ledger row is never re-derived from the current source: refused
  unless an OPERATOR attests; 061 records who and when.
- An emptied ledger over a populated schema is refused as a partial restore.
- W1BR-014's ordering refusal runs before the read-back (same semantics).

Detectors — 13 mutants, 13 killed (`dbdf5bb`; M12 survived first and got its
detector in `0e9c3a8`):

| mutant | killed by |
|---|---|
| no read-back before apply | L4, L4b, objects-removed |
| no read-back after apply | silently-produced-nothing |
| no empty-ledger check | B5 partial restore |
| undigested not refused / pre-057 not refused | L5; pre-057 upgrade |
| backfill every NULL row | pre-057 (attested set wrong) |
| no evidence-integrity check | p4 shape |
| no actor validation | pre-057 (blank actor) |
| 061 CHECK weakened | whole-or-absent |
| no ordering pre-check | 5 W1BR-014 / replay tests |
| drops ignored | 10 tests incl. re-added dropped column |
| function bodies not compared | weakened guard function (the G-08 incident) |
| column types not compared | type-drift test |

**Disclosed limits.** Six migrations have no catalog effect the read-back can
see (029, 032, 056 grants; 048 function `SET` config; 046, 049 same-name
constraint re-adds). Trigger ENABLED state, function `SET` config, ownership
and grants are not read back — the suite's own tests change them temporarily on
a shared database. A disabled trigger is therefore NOT refused by the read-back.

### PRODUCTION CHANGE 2 — `e1b582d`: a second migrator waits instead of crashing

Files: `src/persistence/postgres/migrations.ts` (`takeLedgerLockPatiently`),
`pool.ts` (`MIGRATION_BOUNDS.ledgerLockTotalWaitMs` = 1h).

The ledger lock is retried in `lockTimeoutMs` attempts up to the total, each
retry logged; the session `statement_timeout` is raised above one attempt for
the wait and RESET in the `finally`. On the production pool, a second migrator
held for 125s now FULFILS at 125,149ms (was REJECTED 57014 at 30,018ms).

Detectors — 4 mutants, 4 killed (`c158c17`): no retry, statement bound not
raised (both: the 125s-held migrator test), statement bound not reset
(session-hygiene test), total wait under 12 × `statementTimeoutMs` (bounds
relation test). Disclosed: a migration run longer than the total still makes a
waiter fail closed.

### R2 AGAINST ITS ACCEPTANCE LINES

| id | acceptance | status |
|---|---|---|
| R2.1 | L4, L4b, L5 fail CLOSED; L5-control still refuses | **MET** — premise probe re-run after the change (`d03-premises.after-R2.txt`) and pinned by tests |
| R2.2 | p4 re-executed: edited-055 upgrade REFUSED; a row whose objects are absent REFUSED | absent-objects **MET**; p4 literal re-execution recorded in the next entry |
| R2.3 | MUT-B killed with a named assertion | **MET** — K-06c "the migrator created the ledger BEFORE taking the advisory lock ..." (`1202fc7`) |
| R2.4 | a second migrator against a slow first does not crash at boot | **MET** — bare pool (test) and production pool (probe) |
| R2.5 | comment and behaviour agree; a test pins it | **MET** — W1BR-014 comment states what it does not cover; B5 refused by design, pinned |

Also: the replay test's stale "the lock is gone" comment corrected (I-7).
Manifest 1098 -> 1115 from a clean full suite (`c295b7b`).

### R2.2 — gate 5's p4 probe, RE-EXECUTED LITERALLY (at `c295b7b`)

A pre-057 database built by the UNMODIFIED build, then upgraded by a build with
055's SQL edited by one added comment line (gate 5's edit). Eight cases, each on
its own database (`r2/probes/p4-literal.at-c295b7b.txt`):

| 055 edited | evidence | attestation | result |
|---|---|---|---|
| outside a function body | stale | none / operator | **REFUSED** — evidence not generated from this SQL |
| outside a function body | regenerated | none | **REFUSED** — pre-057 ledger needs an attestation |
| outside a function body | regenerated | operator | **ACCEPTED** — digest of the edited text recorded, attested |
| inside a function body | stale | none / operator | **REFUSED** — evidence not generated from this SQL |
| inside a function body | regenerated | none / operator | **REFUSED** — read-back: 055's function differs |

The edited-055 upgrade is refused wherever the database would NOT hold what the
build says, and wherever nobody attests. The single accepted case is an edit
that changes nothing the database stores (a SQL comment outside any function
body), in a build whose evidence was regenerated from it, attested by a named
operator and recorded (061). **Disclosed residual:** in that case the ledger's
055 digest names the edited text, so the unedited source would afterwards be
refused against it (gate 5's "not recoverable" consequence, narrowed to an
attested, catalog-identical edit). Not silent laundering; not a refused state
either. R2.2 is taken as MET on that reading.

**R2 enforce-mode full suite at `c295b7b`:**

    WATCHDOG VERDICT: PASS scope=FULL_SUITE tests=1115 pass=1115 fail=0 cancelled=0 skipped=0 todo=0
